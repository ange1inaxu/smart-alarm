...
int set_sound(float angle) {
  ...
  tft.println("Please set the alarm sound: ");
  while(true) {
    get_angle(&a, &angle);
    ...
    if (bv1 == 1) {
      ...
      current_num[0] = sound_numbers[char_index];
      /*strcat(alarm_time, current_num);
      strcpy(output, alarm_time);
      char_index = 0;*/
      songNum = atoi(current_num);
      return songNum;
    ...
    numToTitle(atoi(current_num), title);
    //sprintf(output, "%s%c", alarm_time, numbers[char_index]);
    
    /*if(!compare(prev_title, title)) {
...
void numToTitle(int songNum, char* title) {
  switch (songNum) {
    case 1:
      strcpy(title, "Feel Special - TWICE       ");
      break;
    case 2:
      strcpy(title, "Silk                       ");
      break;
    case 3:
      strcpy(title, "22 - Taylor Swift          ");
      break;
  }
}
