...
void loop(){
  ...
  //   }
  // } else if (on_state == ON){
  //   if (acceleration <= 11 && millis() - sleep_time >= BLACKOUT_TIME){
  //     on_state = OFF;
  ...
  if (gotten_mode_state == 0) {
    ...
    if (mode_state == SET_MODE) {
      ...
      tell_time();
      // go back to choosing modes
      // bv4 = button4.update();
      // if (bv4 == 1) {
      //   gotten_mode_state = 0;
      //   tft.fillScreen(TFT_BLACK);
      //   Serial.println("alarm to choosing modes");
      //   time_set = false;
      // }
    }
  }
...
void handle_modes(int bv4) {
  if (on_state == ON){
    ...
    tft.println("  Mode 0: User input");
    tft.println("  Mode 1: Rise&Shine");
    tft.println("  Mode 2: Location   ");
    tft.println("  Mode 3: Calendar   ");
    tft.println("  Mode 4: Voice  ");
